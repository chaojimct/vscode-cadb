...
import {
  ...
  type CadbTableDragData,
} from "./cadb_drag_drop_controller";
...
async function resolveActiveDatabaseDatasource(
  ...
  if (currentDb.dataloader) {
    chatDebug(
      logChannel,
      "resolveActiveDatabase: 使用当前库节点（含 dataloader）",
      {
        targetName,
      },
    );
...
/**
 * 根据拖放传入的 CadbTableDragData，从连接树中解析并构建单张表的字段摘要。
 */
async function buildDraggedTableSchemaText(
  dragData: CadbTableDragData,
  databaseManager: DatabaseManager,
  context: vscode.ExtensionContext,
  token: vscode.CancellationToken,
): Promise<
  { text: string; connectionLabel: string; databaseLabel: string } | undefined
> {
  const provider = databaseManager.provider;

  // 在分组下查找匹配连接名的 datasource 节点
  let connectionNode: Datasource | undefined;
  for (const groupNode of provider.getRootNodes()) {
    for (const child of groupNode.children) {
      if (
        child.type === "datasource" &&
        child.label?.toString() === dragData.connectionName
      ) {
        connectionNode = child;
        break;
      }
    }
    if (connectionNode) {
  ...
  const connObjects = await connectionNode.expand(context);
  const datasourceTypeNode = connObjects.find(
    (o) => o.type === "datasourceType",
  );
  if (!datasourceTypeNode) {
    return undefined;
  }
  ...
  const databaseNode = databases.find(
    (d) => d.label?.toString() === dragData.databaseName,
  );
  if (!databaseNode) {
    return undefined;
  }

  const dbObjects = await databaseNode.expand(context);
  if (token.isCancellationRequested) {
    return undefined;
  }
  ...
  const tableNode = tables.find(
    (t) => t.label?.toString() === dragData.tableName,
  );
  if (!tableNode) {
    return undefined;
  }
  ...
  const lines: string[] = [
    `连接: ${dragData.connectionName}`,
    `数据库: ${dragData.databaseName}`,
    "",
  ];

  const sub = await tableNode.expand(context);
  if (token.isCancellationRequested) {
    return undefined;
  }
  const fieldType = sub.find((o) => o.type === "fieldType");
  if (!fieldType) {
    lines.push(`### ${dragData.tableName}`, "(无字段节点)", "");
  } else {
    ...
    const tableComment =
      typeof tableNode.tooltip === "string" && tableNode.tooltip.trim()
        ? ` — ${tableNode.tooltip.trim()}`
        : "";
    lines.push(`### ${dragData.tableName}${tableComment}`);
    for (const col of columns || []) {
      const cn = col.label?.toString() || "";
      const ctype = (col as Datasource).data?.extra || "";
      const nullable =
        (col as Datasource).data?.nullable === true ? " NULL" : "";
      const cct =
  ...
  return {
    text: lines.join("\n"),
    connectionLabel: dragData.connectionName,
    databaseLabel: dragData.databaseName,
  };
}
...
> {
  ...
  for (const table of tables) {
    ...
    if (n >= MAX_TABLES) {
      lines.push(
        `… 另有 ${tables.length - MAX_TABLES} 张表未展开（已达上限）。`,
      );
      break;
    }
...
export function registerCadbTableChatParticipant(
  ...
  const handler: vscode.ChatRequestHandler = async (
    ...
    // 检测是否有从数据源树拖入的表节点
    let draggedTable: CadbTableDragData | undefined;
    for (const ref of request.references ?? []) {
      if (ref.id === CADB_TABLE_DRAG_MIME_TYPE) {
        try {
          const items = JSON.parse(String(ref.value)) as CadbTableDragData[];
          if (Array.isArray(items) && items[0]) {
            draggedTable = items[0];
          }
        } catch {
          // 忽略解析错误
        }
    ...
    let built: Awaited<ReturnType<typeof buildCurrentDatabaseSchemaText>>;
    if (draggedTable) {
      stream.progress(`正在读取表 \`${draggedTable.tableName}\` 的结构…`);
      built = await buildDraggedTableSchemaText(
        draggedTable,
        databaseManager,
        context,
        token,
      ...
      if (!built) {
        // 降级到当前库全量 schema
        built = await buildCurrentDatabaseSchemaText(
          databaseManager,
          context,
          token,
          logChannel,
    ...
    if (!built) {
      chatDebug(
        logChannel,
        "handler: 无法构建 schema（未选库或解析失败），将返回提示文案",
      );
      stream.markdown(
    ...
    if (request.command === "schema") {
      ...
      stream.markdown(
        formatSchemaMarkdown(connectionLabel, databaseLabel, schemaBody),
      );
      return;
    }
